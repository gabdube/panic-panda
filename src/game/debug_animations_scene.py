...
class DebugAnimationsScene(object):
    ...
    def _setup_assets(self):
        ...
        bunny_o.uniforms.base_material = {"metallic_roughness": (0.0, 1.0)}

        # Packing
        scene.images.extend(bunny_i, brdf_i, env_i, env_irr_i)
        scene.samplers.extend(bunny_sm, brdf_sm, env_sm)
        scene.shaders.extend(pbr_s)
        scene.meshes.extend(bunny_m)
        scene.objects.extend(bunny_o)

        self.shaders = (pbr_s,)
        self.objects = (bunny_o,)
